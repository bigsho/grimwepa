...
// cracks wpa (looks for wpacracked.txt, waits for data to be written to it)
	...
	public void run() {
		...
		do {
			...
			try {
				...
				input =  new BufferedReader(new FileReader("wpacracked.txt"));
		...
		if (cracked == true) {
			// cracked it!
			test2.stat("Cracked WPA Key: " + result);
		} else {
			// didn't crack it.
