...
public class targetClass implements Runnable {
	...
	public void run() {
		...
		try {
			Runtime.getRuntime().exec("rm -rf targets-01.csv");
			test2.pause(0.3);
		} catch(IOException e) {}
		...
		try {
			...
			pro1 = Runtime.getRuntime().exec(
				"xterm -fg green -bg black -bd green -geom 100x15+0+0 " + 
				"-T GrimWepa-TargetScan -iconic " + 
				"-e airodump-ng " + chan + "-w targets --output-format csv " + drv);
		} catch(IOException e) {}
		...
		do {
			...
			for (int i = 0; i < tout; i++) {
				if (tout - i != 1)
					test2.stat("Waiting for " + (tout - i) + " second(s)...");
				else
			...
			try {
				res1 = new Scanner(new FileReader("targets-01.csv"));
			} catch (FileNotFoundException e) {
				System.out.println("Error: FileNotFoundException: File: targets-01.csv");
				res1 = null;
				continue;
			}
