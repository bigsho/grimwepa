	...
	public void run() {
		...
		do {
			...
			try {
				...
				if ( (new File(test2.grimwepaPath + "badpw.txt")).exists() == false) {
					w = new PrintWriter(new FileWriter(test2.grimwepaPath + "badpw.txt"));
					w.println("blahblahblah");
					w.flush();
				}
				
				pro2 = Runtime.getRuntime().exec("aircrack-ng -a 2 -w " + test2.grimwepaPath + 
						"badpw.txt " + test2.grimwepaPath + "wpa-01.cap");
				pro2.waitFor();
		...
		try {
			pro2 = Runtime.getRuntime().exec("rm -rf " + test2.grimwepaPath + "badpw.txt");
			pro2.waitFor();
		...
		if (handshake == true) {
			...
			try {
				pro2 = Runtime.getRuntime().exec("rm -rf " + test2.grimwepaPath + "wpa-01.cap");
				pro2.waitFor();
				pro2.destroy();
