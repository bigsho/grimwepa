...
public class threadWepAttack implements Runnable {
	...
	*/
	public void run() {
		...
		if (Gui.chkWepClient.isSelected() == true && 
			client.equals("[Clientes nao encontrados]") == false) {
		...
		case 1: // arp-replay attack
			...
			do {
				...
				if (!Gui.cboWepClients.getSelectedItem().equals("[Clientes nao encontrados]")) {
	...
	*/
	private static void showIVS() {
		...
		if (count >= 10000 && Gui.btnWepCrack.getLabel().equals("Iniciar cracking") && !started_autocrack) {
