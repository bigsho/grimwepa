...
public class Methods {
	...
	*/
	public static void refreshDrivers() {
		...
		stat("Procurando Receptor Wireless...");
		
		Gui.cboDrivers.removeAllItems();
		Gui.cboDrivers.addItem("Selecione:");
		...
		if (Gui.cboDrivers.getItemCount() == 1) {
			...
			if (ifaces.equals("")) {
				...
				JOptionPane.showMessageDialog(
					null,
					"Receptor Wireless nao encontrado.\n\n" + 
					"Backtrack pode nao ter detectado seu chipset.",
					"Grim Wepa | Erro",
					JOptionPane.ERROR_MESSAGE);
				stat("inactive");
				return;
			...
			String choice = (String)JOptionPane.showInputDialog(null, 
						"Selecione um receptor Wireless para habilitar o Modo Monitor:", 
						"Grim Wepa | Selecione um receptor para Modo Monitor",
			...
			stat("Colocando '" + choice + "' em Modo Monitor...");
			Gui.cboDrivers.removeAllItems();
			Gui.cboDrivers.addItem("Selecione:");
	...
	*/
	public static void targetScanStart() {
		...
		if (driver.equals("Selecione:")) {
			JOptionPane.showMessageDialog(
				null,
				"Selecione um receptor para iniciar a busca.",
				"Grim Wepa | Erro",
				JOptionPane.ERROR_MESSAGE
			);
			return;
		...
		Gui.btnTargets.setLabel("Parar Busca");
	...
	*/
	public static void targetScanStop() {
		...
		Gui.btnTargets.setLabel("Buscar Redes");
		Gui.setEnable(true);
	}
	...
	*/
	public static void wepCrack() {
		if (Gui.btnWepCrack.getLabel().equals("Iniciar cracking")) {
			Gui.btnWepCrack.setLabel("Parar Cracking");
			...
			// Gui.btnWepCrack.setLabel("Iniciar cracking");
	...
	*/
	public static void wepAttack() {
		...
		if (Gui.btnWepAttack.getLabel().equals("Iniciar Ataque") == true) {
			// make sure they selected a valid attack
			if (Gui.cboWepAttack.getSelectedIndex() == 0) {
				JOptionPane.showMessageDialog(
					null,
					"Selecione um metodo de ataque!",
					"grim wepa | Ataque WEP",
					JOptionPane.ERROR_MESSAGE
				);
				return;
			}
			...
			// capture!
			Gui.btnWepAttack.setLabel("Parar Ataque");
			...
			Gui.btnWepAttack.setLabel("Iniciar Ataque");
	...
	*/
	public static void wpaAttack() {
		...
		if (Gui.btnWpaDeauth.getLabel().equals("Iniciar captura handshake")) {
			// if we are supposed to be deauthing...
			
			Gui.btnWpaDeauth.setLabel("Para captura handshake");
			Gui.setEnable(false);
			...
			// user clicked STOP
			Gui.setEnable(true);
			Gui.btnWpaDeauth.setLabel("Iniciar captura handshake");
	...
	*/
	public static void wpaCrackDictionary() {
		if (Gui.btnWpaCrack.getLabel().equals("Crack com WPA...")) {
			// user selected to begin cracking
			Gui.btnWpaCrack.setLabel("Iniciar cracking");
			...
			Gui.btnWpaCrack.setLabel("Crack com WPA...");
	...
	*/
	public static void wpaCrackPyrit() {
		if (Gui.btnWpaCrack.getLabel().equals("Crack com WPA...")) {
			// user selected to begin cracking
			Gui.btnWpaCrack.setLabel("Parar cracking");
