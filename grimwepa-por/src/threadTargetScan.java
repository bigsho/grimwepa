	...
	public void run() {
		...
		do {
			...
			for (int i = 0; i < tout; i++) {
				...
				if (exitVal != -32767 || Gui.btnTargets.getLabel().equals("Buscar Redes"))
					break;
			}
		...
		} while (Gui.btnTargets.getLabel().equals("Buscar Redes") == false);
		...
		if (Gui.cboWepClients.getItemCount() == 0) {
			Gui.cboWepClients.addItem("[Clientes nao encontrados]");
		}
		if (Gui.cboWpaClients.getItemCount() == 0) {
			Gui.cboWpaClients.addItem("[Clientes nao encontrados]");
		}
