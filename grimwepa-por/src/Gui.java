...
public class Gui extends Frame implements WindowListener, ActionListener, EventListener, ListSelectionListener {
	...
	*/
	public void buildControls() {
		...
		// ROW #1: FOR INTERFACE
		// install button
		btnInstall = new Button("Instalar");
		...
		//cboDrivers.setPreferredSize(new Dimension(85, 20));
		cboDrivers.setFont(f);
		cboDrivers.addItem("selecione:");
		add(cboDrivers);
		cboDrivers.addActionListener(this);
		// 'refresh drivers' button
		btnDrivers = new Button("Atualizar Receptores");
		...
		tabTargets.setPreferredScrollableViewportSize(new Dimension(420,200));
		tabTargets.setFillsViewportHeight(true);
		tabTargets.getColumnModel().getColumn(0).setPreferredWidth(20);   // power
		tabTargets.getColumnModel().getColumn(1).setPreferredWidth(100);  // SSID
		tabTargets.getColumnModel().getColumn(2).setPreferredWidth(20);   // channel
		tabTargets.getColumnModel().getColumn(3).setPreferredWidth(35);   // encryption
		...
		add(sldChannel);
		chkChannel = new JCheckBox("Todos Canais");
		...
		btnKeyTank = new Button("Senhas Armazenadas (0)");
		btnKeyTank.setBackground(Color.red);
		btnKeyTank.setForeground(Color.black);
		btnKeyTank.setPreferredSize(new Dimension(150,22));
		add(btnKeyTank);
		...
		btnTargets = new Button("Buscar Redes");
		...
		// target timeout
		lblTargetTimeout = new JLabel("Timeout (seg):");
		...
		// ROW #5: WEP PANEL
		panWep = new JPanel();
		panWep.setPreferredSize(new Dimension(430,155));
		// client checkbox
		chkWepClient = new JCheckBox("Usar cliente no ataque:", false);
		...
		cboWepClients.addItem("[Clientes nao encontrados]");
		...
		btnWepTestinj = new Button("Teste Injecao");
		...
		lblWepInjection = new JLabel("Taxa de Injecao (pps):");
		...
		panWpa = new JPanel();
		panWpa.setPreferredSize(new Dimension(430, 155));
		...
		// clients checkbox
		chkWpaClients = new JCheckBox("Usar cliente no ataque:");
		...
		cboWpaClients.addItem("[Clientes nao encontrados]");
		cboWpaClients.setPreferredSize(new Dimension(150, 20));
		...
		// start attack button
		btnWpaDeauth = new Button("Iniciar captura handshake ");
		...
		// timeout label
		lblWpaTimeout = new JLabel("Atraso (sec):");
		...
		// warning label
		lblWpaWarning = new JLabel("[Cracking somente apos handshake capturado]");
		...
		// crack button
		btnWpaCrack = new Button("Crack com WPA ...");
		...
		txtWpaWordlist = new JTextField("[Wordlist Padrao]");
		...
		cboWpaCrackMethod.addItem("Select um metodo:");
	...
	*/
	public void actionPerformed(ActionEvent event) {
		if (event.getSource() == cboColors) {
			...
			if (btnTargets.getLabel().equals("Buscar Redes"))
			...
			GuiPWTank gpwt = new GuiPWTank("Grim Wepa | Senhas Armazenadas");
			Main.guiWindow.setVisible(false);
		
		} else if (event.getSource() == btnInstall) {
			// user clicked 'install' button
			Main.guiWindow.setVisible(false);
			GuiInstall gi = new GuiInstall("Grim Wepa | Instalar");
	...
	*/
	public void valueChanged(ListSelectionEvent e) {
		...
		if (temp.indexOf("WEP") >= 0) {
			...
			// build the border
			panWep.setBorder(BorderFactory.createTitledBorder(
						null, "WEP | Redes '" + (String)tabTargets.getValueAt(row,4) + "'" + key, 
						0, 0, null, 
			...
			panWpa.setBorder(BorderFactory.createTitledBorder(
				null, "WPA | Redes '" + (String)tabTargets.getValueAt(row,4) + "'" + key, 
	...
	public void windowClosing(WindowEvent e) {
		...
		if (!Methods.putInMonitorMode.equals("") && Methods.isValidDriver(Methods.putInMonitorMode)) {
			...
			if (JOptionPane.showConfirmDialog(
				null,
				"O Receptor '" + Methods.putInMonitorMode + "' esta no Modo Monitor.\n\n" +
				"Deseja Habilitar o Modo Monitor?",
				"Grim Wepa | Saindo",
		...
		if (!btnTargets.getLabel().equals("Buscar Redes") && Methods.targetScan.t.isAlive()) {
			btnTargets.setLabel("Buscar Redes");
			try {
